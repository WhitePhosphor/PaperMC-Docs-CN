...
每个命令类都需要实现一个 <Javadoc name={"com.velocitypowered.api.command.Command"} project={"velocity"}>`Command`</Javadoc> 子接口。
...
Brigadier 命令完全向后兼容 1.12.2 及更低版本。

Velocity 不支持自定义插件命令参数类型，因为这需要客户端也支持它们。我们建议坚持使用提供的预定义 Brigadier 类型。
...
一个用于检查 <Javadoc name={"com.velocitypowered.api.command.CommandSource"} project={"velocity"}>`CommandSource`</Javadoc>
是否有权限使用该命令。所有方法都接收一个
<Javadoc name={"com.velocitypowered.api.command.SimpleCommand$Invocation"} project={"velocity"}>`SimpleCommand.Invocation`</Javadoc>
对象，其中包含执行命令的 `CommandSource` 和作为字符串数组的参数。
前面的例子也可以使用这个接口实现：
...
重要的是要注意 <Javadoc name={"com.velocitypowered.api.command.CommandInvocation#arguments()"} project={"velocity"}>`invocation.arguments()`</Javadoc>
不包括命令别名（例如，`/teleport foo bar` 中的 `teleport`）。如果没有指定参数，将传递一个空数组，而不是 null 数组。

如果玩家或控制台执行以下命令：`/stats Player2 kills`，第一个参数将是 `Player2`，我们可以使用 `invocation.arguments()[0]` 访问它，
第二个参数将是 `kills`。

### <Javadoc name={"com.velocitypowered.api.command.RawCommand"} project={"velocity"}>`RawCommand`</Javadoc>

在某些情况下，你不需要处理参数。这些可能包括：

- 类似 `/say` 的命令，其中参数包含作为字符串的消息；或者
- 你正在使用外部命令框架来处理你的命令。

原始命令指示代理服务器直接传递命令别名及其参数，而不进行进一步处理。让我们看一个回显接收到的输入的命令示例：
...
现在我们已经创建了一个命令，我们需要注册它才能工作。要注册命令，你需要使用
<Javadoc name={"com.velocitypowered.api.command.CommandManager"} project={"velocity"}>`CommandManager`</Javadoc>。
我们通过执行 <Javadoc name={"com.velocitypowered.api.proxy.ProxyServer#getCommandManager()"} project={"velocity"}>`proxyServer.getCommandManager()`</Javadoc>
获取命令管理器，或者通过在主类中使用 [`@Inject`](https://javadoc.io/doc/com.google.inject/guice/latest/com/google/inject/Inject.html)
注解注入它。注册方法需要两个参数，命令元数据和命令对象。

<Javadoc name={"com.velocitypowered.api.command.CommandMeta"} project={"velocity"}>`CommandMeta`</Javadoc>
包含不区分大小写的别名和更高级的功能。
`CommandManager` 通过 <Javadoc name={"com.velocitypowered.api.command.CommandManager#metaBuilder(java.lang.String)"} project={"velocity"}>`#metaBuilder(String alias)`</Javadoc>
方法提供元数据构建器。
...
如果你正在注册一个 `BrigadierCommand`，你可能更喜欢使用
...
方法或 <Javadoc name={"com.velocitypowered.api.command.CommandManager#metaBuilder(com.velocitypowered.api.command.BrigadierCommand)"} project={"velocity"}>`#metaBuilder(BrigadierCommand)`</Javadoc>
来指定额外的别名。